...
### Temas aplicados dentro del proyecto:
- Análisis de redes y medidas de centralidad
- Detección de comunidades -
- Descomposición en Valores Signulares
...
### Análisis conjunto (todos los libros)  Betwenness & Eigen: 
...
nodes <- tidygraph::as_tbl_graph(data_all, directed = FALSE) %>% 
  ...
  as_tibble() %>% 
  mutate(label = name) %>% 
  select(id,  label, title, group, name, value_cty, value_btw, value_cls, value_eigen)


DT::datatable(nodes %>% mutate_if(is.numeric, round, 3))
...
mat_cor <- cor(nodes %>% select(-id, -label, -title, -name, -group))
...
```{r}
comps_1 <- prcomp(nodes %>% select(-id, -label, -title, -name, -group), scale = TRUE)
...
##  Detección de comunidades y estimación de importancia
