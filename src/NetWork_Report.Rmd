...
nodes <- tidygraph::as_tbl_graph(data_all, directed = FALSE) %>% 
  ...
  as_tibble() %>% 
  rename(label = name) %>% 
  select(id,  value_cty, value_btw, value_cls, value_eigen)
...
corrplot(mat_cor, main = "Matriz de correlación",
         method = "number",
         tl.pos = "top",
         mar = c(2, 1, 3, 1)) 
...
fviz_eig(comps_1 ,addlabels=TRUE, hjust = -0.3,  barfill="white", linecolor ="red") + theme_minimal()
```
...
nodes <- nodes %>% mutate(value_sdv_sc = round(base::scale(value_sdv),0) )
nodes
```
